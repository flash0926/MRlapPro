...
run_LDSC <- function(exposure_data,
  ...
  #data.table::fwrite(exposure_data, paste0(exposure_name, ".tsv"), sep="\t", quote=F, row.names=F)
  #data.table::fwrite(outcome_data, paste0(outcome_name, ".tsv"), sep="\t", quote=F, row.names=F)


  if(verbose) cat("> Munging exposure data... \n")
  invisible(utils::capture.output(.munge(exposure_data,
                                              hm3,
                                              exposure_name)))
  ...
  invisible(utils::capture.output(.munge( outcome_data,
                                              hm3,
                                              outcome_name)))
